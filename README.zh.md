...
- 凭证记账：新增/编辑采用在线凭证式所见即所得录入，默认 5 行分录，可继续新增；空白行不保存，有内容行必须完整校验。
...
- ratel 助手 AI写作：发送前通过下拉选择 `思考` 或 `AI写作`，默认 `思考`；AI写作支持生成 xlsx、docx、pdf、pptx，采购清单、按日统计和经营分析报告复用当前账套真实业务指标。
- ratel 助手深度思考：`思考` 意图统一走推理模型场景；回答生成前展示阶段化思考过程，最终结论返回后自动移除过程提示。
