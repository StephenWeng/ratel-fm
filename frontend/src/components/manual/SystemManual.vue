  ...
  <div class="manual">
    <section class="manual-hero">
      <div>
        ...
        <p>面向日常业务用户，说明登录、数据维护、审批、财务、采购、物流、库存、应收应付、业务 Agent 和智能检索的常用操作。</p>
      </div>
...
const manualSections: ManualSection[] = [
  {
    ...
    items: [
      ...
      {
        title: '登录失败提示',
        description: '所属公司、账号、身份证号或密码不正确时，登录页会直接提示失败原因。',
        steps: [
          '确认当前选择的所属公司是否正确。',
          '确认登录账号或身份证号属于该所属公司。',
          '重新输入密码后再次登录。',
          '连续失败时联系管理员核对账号状态或重置密码。'
        ],
        note: '为避免暴露账号是否存在，账号不存在和密码错误通常会统一提示。'
      },
      {
        title: '顶部工具区',
  ...
  },
  {
    ...
    items: [
      ...
      {
        title: '会计平台',
        description: '会计平台用于汇总采购、库存、应收应付、出纳流水和凭证相关检查。',
        steps: [
          '进入“财务管理 / 会计平台”。',
          '按项目、供应商、客户、物料或期间查看业务链路数据。',
          '需要智能分析时，可到“AI 助手 / 业务 Agent”输入会计平台相关问题。',
          '需要制证时，先核对 Agent 草稿和证据，再由有权限人员按正式凭证流程保存。'
        ],
        note: 'Agent 给出的凭证建议是草稿，不等同于已保存凭证。'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      {
        title: 'ratel 助手',
        description: '页面右下角悬浮图标可打开 ratel 助手，默认使用本地知识库回答系统内问题，并能识别部分业务 Agent 意图。',
        steps: [
          '点击右下角系统图标打开对话框。',
          '输入问题，例如“查询某采购单状态”或“这个月有多少起物流运输”。',
          '回答中出现“进入”按钮时，点击可跳转到目标模块并带上搜索条件。',
          '输入对账、到期、制证建议、库存风险等问题时，助手会自动切换到业务 Agent 并运行分析。'
        ],
        note: '助手会尽量基于系统数据和知识库回答；没有数据依据时会提示原因和建议查询路径。'
      },
      {
        title: '业务 Agent',
        description: 'AI 助手页提供“业务 Agent”页签，用于执行面向业务链路的专项分析。',
        steps: [
          '进入“AI 助手”，切换到“业务 Agent”页签。',
          '输入自然语言问题，例如“检查采购、收货、库存、应付、付款和凭证是否一致”。',
          '按需要选择查询型、对账检查、凭证建议、到期提醒、流程助手、库存风险、经营分析或附件/知识问答。',
          '查看能力结果、模块证据、自检结果和后续建议。'
        ],
        note: '业务 Agent 受系统开关控制。关闭后入口隐藏，前端封装接口也不会调用后端 Agent。'
      },
      {
        title: '智能检索',
        description: '智能检索用于跨模块查询业务数据、附件文本和知识索引。',
        steps: [
          '进入“智能检索”。',
          '输入自然语言问题或单号、供应商、物料等关键词。',
          '可选择“混合”“关键词”或“语义”模式。',
          '系统会展示类型、编码/单号、内容和评分，命中词会高亮。',
          '结果较多或单条内容较长时，在结果表格内部滚动查看后续记录。'
        ]
      },
      {
        title: 'AI 组件状态',
        description: 'AI 组件状态页用于查看大模型、向量库、知识索引、流式输出和业务 Agent 的可用状态。',
        steps: [
          '进入“AI 组件状态”。',
          '查看当前 provider、索引分片数量和各组件状态。',
          '当业务 Agent 关闭时，状态卡会显示入口隐藏和调用阻断。',
          '排查异常时，可先确认 Qdrant、Ollama、embedding、知识索引和业务 Agent 是否正常。'
        ]
      },
      {
        title: '统计报表和操作日志',
